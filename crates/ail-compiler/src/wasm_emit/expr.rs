...
use self::calls::{emit_call_expr, emit_effect_call_expr};
use self::fold::emit_fold_expr;
...
pub(super) fn emit_anf_expr<'a>(
    ...
    match expr {
        ...
        AnfExpr::Call { func, args } => emit_call_expr(func, args, ctx, functions, insns),

        AnfExpr::EffectCall {
            capability,
            func,
            args,
        } => emit_effect_call_expr(capability, func, args, ctx, insns),
        ...
        AnfExpr::Fold { init, list, func } => {
            emit_fold_expr(init, list, func, ctx, functions, insns)
        }
