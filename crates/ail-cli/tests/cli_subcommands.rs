...
fn compile_file_rejects_source_int_bounds_helper_type_mismatch() {
    ...
    source
        .write_str(
            "fn bounded(value: Int, fallback: Text) -> Int = int_wrapping_mul(value, fallback)\n",
        )
        .expect("source fixture must be written");
    ...
    ail()
        ...
        .failure()
        .stderr(predicate::str::contains(
            "type mismatch in int.wrapping_mul argument 2: expected Int, got Text",
        ));
}
...
fn lsp_diagnose_accepts_source_int_bounds_helpers() {
    ...
    source
        .write_str(
            "fn bounded(value: Int, low: Int, high: Int) -> Int = int_min(value, high) + int_max(value, low) + int_clamp(value, low, high) + int_abs_or(value, 0) + int_neg_or(value, 0) + int_add_or(value, 1, 0) + int_sub_or(value, 1, 0) + int_mul_or(value, 1, 0) + int_saturating_add(value, 1) + int_saturating_sub(value, 1) + int_saturating_mul(value, 1) + int_saturating_neg(value) + int_wrapping_add(value, 1) + int_wrapping_sub(value, 1) + int_wrapping_mul(value, 1) + int_div_or(value, 1, 0) + int_rem_or(value, 1, 0)\n",
        )
        .expect("source fixture must be written");
