...
fn run_file_executes_ail_source_int_bounds_helpers() {
    ...
    source
        .write_str(
            "fn bounded() -> Int = int_min(10, -2) + int_max(10, -2) + int_clamp(42, 0, 10) + int_abs_or(-7, 0) + int_abs_or(-9223372036854775808, 99) + int_neg_or(-5, 0) + int_neg_or(-9223372036854775808, 17) + int_add_or(40, 2, -1) + int_add_or(9223372036854775807, 1, 19) + int_sub_or(50, 8, -1) + int_sub_or(-9223372036854775808, 1, 23) + int_div_or(21, 3, -1) + int_div_or(1, 0, 5) + int_div_or(-9223372036854775808, -1, 11) + int_rem_or(22, 5, -1) + int_rem_or(1, 0, 6) + int_rem_or(-9223372036854775808, -1, 13)\n",
        )
        .expect("source fixture must be written");
    ...
    ail()
        ...
        .stdout(predicate::str::contains("result: 316"));
}
...
fn compile_file_accepts_source_int_bounds_helpers() {
    ...
    source
        .write_str(
            "fn bounded(value: Int, low: Int, high: Int, fallback: Int) -> Int = int_sub_or(int_add_or(int_neg_or(int_rem_or(int_div_or(int_abs_or(int_clamp(int_min(value, high), low, high), 0), 2, fallback), 3, fallback), fallback), 1, fallback), 1, fallback)\n",
        )
        .expect("source fixture must be written");
...
fn compile_file_rejects_source_int_bounds_helper_type_mismatch() {
    ...
    source
        .write_str(
            "fn bounded(value: Int, fallback: Text) -> Int = int_sub_or(value, 2, fallback)\n",
        )
        .expect("source fixture must be written");
    ...
    ail()
        ...
        .failure()
        .stderr(predicate::str::contains(
            "type mismatch in int.sub_or argument 3: expected Int, got Text",
        ));
}
...
fn lsp_diagnose_accepts_source_int_bounds_helpers() {
    ...
    source
        .write_str(
            "fn bounded(value: Int, low: Int, high: Int) -> Int = int_sub_or(int_add_or(int_neg_or(int_rem_or(int_div_or(int_abs_or(int_clamp(int_min(value, high), low, high), 0), 2, 0), 3, 0), 0), 1, 0), 1, 0)\n",
        )
        .expect("source fixture must be written");
