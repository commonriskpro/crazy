...
fn known_source_builtin_arity(call: &str) -> Option<SourceArity> {
    let arity = match call {
        ...
        | "int_min" | "int_max" | "int_abs_or" | "int_neg_or" => SourceArity::Exact(2),
        ...
        | "int.rem_or" | "int.sub_or" | "int_clamp" | "int_add_or" | "int_sub_or"
...
fn infer_source_call_type(
    ...
    match func {
        ...
        | "int.abs_or" | "int.neg_or" => {
            validate_source_arg_types(func, args, scope, functions, &["Int", "Int"])?;
            Ok("Int".to_string())
        }
        "int.clamp" | "int.add_or" | "int.sub_or" | "int.div_or" | "int.rem_or" => {
...
fn lower_source_int_bounds_expr(expr: &str, line_num: usize) -> Result<Option<String>, CliError> {
    ...
    let expected_len = if matches!(
        func.as_str(),
        "int_clamp" | "int_add_or" | "int_sub_or" | "int_div_or" | "int_rem_or"
    ) {
        3
    } else {
        2
