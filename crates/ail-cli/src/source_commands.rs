...
fn known_source_builtin_arity(call: &str) -> Option<SourceArity> {
    let arity = match call {
        ...
        | "int.bit_and" | "int.bit_or" | "int.bit_xor" | "int_bit_and" | "int_bit_or"
        | "int_bit_xor" => SourceArity::Exact(2),
...
fn infer_source_call_type(
    ...
    match func {
        ...
        | "int.bit_and" | "int.bit_or" | "int.bit_xor" => {
            validate_source_arg_types(func, args, scope, functions, &["Int", "Int"])?;
            Ok("Int".to_string())
        }
        "int.saturating_neg" | "int.wrapping_neg" => {
...
fn lower_source_int_bounds_expr(expr: &str, line_num: usize) -> Result<Option<String>, CliError> {
    ...
    let expected_len = if matches!(func.as_str(), "int_saturating_neg" | "int_wrapping_neg") {
        1
    } else if matches!(
        func.as_str(),
