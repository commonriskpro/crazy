...
//
// Dispatch entry point: `cmd_package`.  All sub-command handlers live here,
// together with helpers for install, verify, audit, and compatibility logic.
...
use crate::package_output::{
    InstalledPackage, PackageAuditIssue, PackageCompatibilityCliIssue, PackageInstallResult,
    RegistryPackageIntegrity, VerificationReportHashMismatch, advisory_to_json,
    emit_package_compatibility_blocked, format_warnings_for_human, lockfile_entry_to_json,
    package_compatibility_issue_to_json, package_manifest_to_json, reproducible_evidence_status,
...
fn parse_advisory_severity(raw: &str) -> Result<ail_package::AdvisorySeverity, CliError> {
