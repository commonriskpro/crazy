...
| `Fold` | **Implemented** — `call_indirect` loop over `[count:i64, elem:i64, ...]` list; adds funcref `TableSection` (N slots) + active `ElementSection` + fold-reducer type `(i64, i64) → i64`; validates. `func` resolves as top-level function name (table idx = func_idx − offset), local I32 closure env (loads fn_idx from offset 0), or local I64. Nested lambda hoisting deferred. |
