...
| `Bytes` type | **Implemented for WASM and native** — `LiteralValue::Bytes(Vec<u8>)` interns the byte buffer in the WASM data section (WASM) or a `__ail_bytes_N` local data object (native) and emits a packed `(len << 32) \| ptr` i64 return; `derive_wasm_type` maps it to `WasmTypeDescriptor::Bytes`; `infer_cranelift_return_type` returns `I64` for the native path; runtime decodes via `ValueLayout::Bytes` → `StructuredValue::Bytes { ptr, len }`; see [ABI value contract](abi-value-contract.md) |
