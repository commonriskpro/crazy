...
| `Fold` | **Implemented** — `call_indirect` loop over `[count:i64, elem:i64, ...]` list; adds funcref `TableSection` (N slots) + active `ElementSection` + fold-reducer type `(i64, i64) → i64`; validates. `func` resolves as: (1) top-level function name → table idx = func_idx − offset; (2) local I64 → `i32.wrap_i64` + `call_indirect`; (3) local I32 closure env (non-hoistable) → runtime trap (Unreachable guard). |
| `Lambda` (nested, hoistable) | **Implemented (Wave 12A)** — nested `Lambda` with exactly 2 params and no captures (fold-reducer shape `(i64, i64) → i64`) is hoisted into a separate WASM function and emits `i64.const <table_idx>` so Fold can dispatch it via path (2) above. Table size = `n_bindings + n_hoisted`. Hoisted bodies are emitted after all binding functions in the code section and appear in the element section but not in the export section. Supported shape: `params.len() == 2 && captures.is_empty()`. Not yet supported (emits closure env with fn_idx=0 placeholder, Fold traps): Lambdas with captures or ≠ 2 params. |
