...
- `expr_parser.rs` parses the prefix-form executable subset (expanded in `feat/expr-parser-expand`):
  ...
  - **Iteration**: `foreach(binding, collection, body)` — WASM emit is implemented (inline loop, no call_indirect). `fold(init, list, func)` — WASM emit implemented via `call_indirect` + function table (Wave 11B); adds a `TableSection` (funcref, N slots) and `ElementSection` (populates table with all function indices), plus a fold-reducer type `(i64, i64) → i64` in the type section. Wave 12A: nested Lambdas with 2 params and no captures are hoisted into the table so `func` can reference them correctly.
  - **Cells**: `cell_new(init)`, `cell_get(cell)`, `cell_set(cell, value)`.
- `wasm.rs` emits real bodies for simple values, control flow, effect calls, cells (`CellNew`/`CellGet`/`CellSet`), collection constructors (`MapNew`, `SetNew`), indexed access (`IndexGet`), `ForEach` (inline loop over length-prefixed list), and **`Fold`** (via `call_indirect` + function table + element section, Wave 11B); `Dispatch`, all task primitives (`TaskSpawn`/`TaskAwait`/`TaskCancel`/`TaskGroup`), and all channel/concurrency primitives (`ChannelNew`/`ChannelSend`/`ChannelReceive`/`Select`/`Timeout`) are rejected at compile time with `CompileError::UnsupportedWasmConstruct` via a pre-flight walker in `emit_wasm_with_profile`.
...
| `Fold` | **Implemented** — `call_indirect` loop over `[count:i64, elem:i64, ...]` list; adds funcref `TableSection` (N slots) + active `ElementSection` + fold-reducer type `(i64, i64) → i64`; validates. `func` resolves as: (1) top-level function name → table idx = func_idx − offset; (2) local I64 → `i32.wrap_i64` + `call_indirect`; (3) local I32 closure env (captured Lambda) → **compile-time diagnostic** `CompileError::UnsupportedWasmConstruct("FoldWithCapturedReducer")` (Wave 13B). |
| `Lambda` (nested, hoistable) | **Implemented (Wave 12A)** — nested `Lambda` with exactly 2 params and no captures (fold-reducer shape `(i64, i64) → i64`) is hoisted into a separate WASM function and emits `i64.const <table_idx>` so Fold can dispatch it via path (2) above. Table size = `n_bindings + n_hoisted`. Hoisted bodies are emitted after all binding functions in the code section and appear in the element section but not in the export section. Supported shape: `params.len() == 2 && captures.is_empty()`. |
| `Lambda` (nested, captured) | **Compile-time diagnostic (Wave 13B)** — nested `Lambda` with non-empty captures used as a `Fold` reducer returns `CompileError::UnsupportedWasmConstruct("FoldWithCapturedReducer")`. Captured values are not representable as `(i64, i64) → i64` WASM parameters; general closure hoisting (arbitrary arity + captures) is deferred to a future wave. Nested captured Lambdas NOT used as a Fold reducer continue to emit a closure env pointer in linear memory. |
...
| `Bytes` type | **Implemented for WASM and native** — `LiteralValue::Bytes(Vec<u8>)` interns the byte buffer in the WASM data section (WASM) or a `__ail_bytes_N` local data object (native) and emits a packed `(len << 32) \| ptr` i64 return; `derive_wasm_type` maps it to `WasmTypeDescriptor::Bytes`; `infer_cranelift_return_type` returns `I64` for the native path; (WASM only) runtime decodes via `ValueLayout::Bytes` → `StructuredValue::Bytes { ptr, len }`; native emits the same packed i64 in object data but has no ail-runtime decode path; see [ABI value contract](abi-value-contract.md) |
