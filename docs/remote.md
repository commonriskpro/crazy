...
`ail remote push --root <object-id> [--json]` y `ail remote pull <root> [--json]` implementan un slice mínimo y honesto de bundles para proyectos inicializados con store de archivos. `push` carga solo el objeto root indicado desde `.ail/store/objects/`, arma un `ObjectBundle` de un objeto, lo valida a través de `RemoteExchangeRequest::PushBundle`, y lo persiste con `FileBundleStore` en `.ail/remote/bundles/<root>.cbor`. `pull` lee ese bundle local, lo revalida con el exchange in-process (`PushBundle` + `PullBundle`), y escribe los objetos en el object store local. El transporte se reporta como `local_file_bundle_store+in_process` y el alcance actual es `single_root_object`: no hay red, discovery, uso de `.ail/remote.json` en push/pull, ni traversal de dependencias todavía.
