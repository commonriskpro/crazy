...
Cada entrada está indexada por el hash BLAKE3 de sus bytes. `verify_integrity()` re-deriva el `ObjectId` esperado de cada valor y lo compara contra la clave almacenada. Si el objeto raíz decodifica como `SnapshotEnvelope`, también exige que el bundle contenga todas las dependencias CAS directas declaradas por esa envelope; un snapshot incompleto no se trata como completamente confiable. `parent_id` no cuenta como dependencia CAS directa: es identidad de la envelope padre (`SnapshotEnvelope.id`), no necesariamente la clave content-addressed de un objeto en `ObjectStore`.
...
| `BundleError::MissingSnapshotDependency { dependency }` | El root es un `SnapshotEnvelope` y falta una dependencia CAS directa declarada |
...
`ObjectBundle::from_store_with_snapshot_dependencies(root, store)` construye un bundle desde un objeto raíz y, si los bytes raíz decodifican como `SnapshotEnvelope`, agrega las dependencias CAS directas que la envelope declara: `graph_root_hash`, `applied_change_id`, `audit_record_ids` y `migration_metadata_ids`. Si una dependencia CAS declarada no existe en el store local, la construcción falla en vez de producir un bundle parcial. `parent_id` se conserva como metadata de identidad de snapshot para el grafo, pero no se busca directamente en `ObjectStore` salvo que exista una futura asociación explícita identity -> CAS. Los objetos de grafo siguen siendo opacos: no se promete traversal interno de `SemanticGraph` ni traversal transitivo general.
