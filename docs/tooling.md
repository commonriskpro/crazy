...
| `remote push` / `remote pull` | Implemented as `ail remote push --root <object-id> [--json]` and `ail remote pull <root> [--json]` for initialized file-backed projects. Raw roots are bundled alone and report `bundle_scope=single_root_object`. Roots that decode as `SnapshotEnvelope` include available directly referenced stored objects (`graph_root_hash`, parent/change/audit/migration ids) and report `bundle_scope=root_with_snapshot_envelope_dependencies` only when at least one dependency is included. Bundles are persisted under `.ail/remote/bundles/<root>.cbor` and checked through the in-process bundle exchange boundary. It does not claim network transport, remote discovery, remote config loading for push/pull, raw graph traversal, or general transitive traversal. |
