...
| `remote push` / `remote pull` | Implemented as `ail remote push --root <object-id> [--json]` and `ail remote pull <root> [--json]` for initialized file-backed projects. The current bundle scope is one root object only, persisted under `.ail/remote/bundles/<root>.cbor` and checked through the in-process bundle exchange boundary. It does not claim network transport, remote discovery, remote config loading for push/pull, or dependency traversal. |
