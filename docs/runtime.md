...
| Secret vault | Provider abstraction implemented. `SecretProvider` (`crates/ail-runtime/src/secret.rs`) is a `Send + Sync` trait with a single `resolve(vault_path) -> Option<Vec<u8>>` method. `SecretVault` (in-memory `HashMap`) implements it; future adapters (HashiCorp Vault, AWS Secrets Manager, etc.) implement the same trait without touching handler or host code. `SecretReadHandler::new` is generic over `P: SecretProvider + 'static`, so existing call sites (`Arc::new(vault)` where `vault: SecretVault`) compile unchanged. `SecretReadHandler` resolves the logical ID through `RuntimeProfile::secrets_mapping` (id → vault_path), fetches from the provider, and returns the raw bytes to the caller. Audit events record only BLAKE3 hashes of input/output — secret values never appear in audit logs. Unmapped IDs and missing vault paths both return `HostError::CapabilityDenied` without distinguishing which step failed. Grant checks apply on both the host-side path (`RuntimeHost::call_capability`) and the WASM-side path (`ail/host_call` / `ail/host_call_write`); the full dispatch pipeline (WASM → dispatch → handler → provider → WASM memory) is covered by `crates/ail-runtime/tests/secret_wasm_e2e_tests.rs`. External vault adapter implementations remain future work. |
