...
denial_category        (opaque; set only on handler-level denials)
```

Sensitive payloads no se loguean crudos por default. Se loguean hashes/redacted views según policy.

`denial_category` carries a generic failure category string (e.g. `"secret.not_found"`, `"secret.provider_unavailable"`) set by the handler when it can distinguish failure modes. It MUST NOT contain secret IDs, vault paths, or any data that could serve as a vault-layout oracle. Callers always receive an opaque `CapabilityDenied`; the category is audit-only.
...
Implementation note: an in-memory vault (`SecretVault`) and a `secret.read` capability handler (`SecretReadHandler`) are implemented in `crates/ail-runtime/src/secret.rs`. The handler resolves logical secret IDs through `RuntimeProfile::secrets_mapping` (id → vault_path), fetches from the provider, and returns bytes to the caller without logging them (audit records only BLAKE3 hashes). Grant enforcement by `RuntimeHost::call_capability` and the WASM-side dispatch (`ail/host_call` / `ail/host_call_write`) both apply before the handler is reached. Secret bytes can be injected into WASM linear memory via the `ail/host_call_write` import; end-to-end coverage of this path (WASM module → dispatch → handler → vault → memory) is in `crates/ail-runtime/tests/secret_wasm_e2e_tests.rs`.

`SecretProvider::resolve` returns `Result<Vec<u8>, SecretProviderError>` (not found → `Err(NotFound)`, transient failure → `Err(Unavailable)`). The handler maps these to audit categories: both `NotFound` and a mapping miss produce `"secret.not_found"`; `Unavailable` produces `"secret.provider_unavailable"`. All callers receive only opaque `HostError::CapabilityDenied("secret access denied")`; the `denial_category` field is audit-only. A real external vault client remains future work.
...
| Typed boundary codec | `RuntimeInstance::invoke_typed(export, args, ValueLayout)` returns a `StructuredValue` by reading WASM linear memory and decoding with `ValueDecoder`. The milestone codec supports `Scalar`, `Record`, `Variant`, `List`, `Option`, `Result`, and `Handle` layouts; full ABI/value-layout parity remains future validation work. **Wave 14B** added `schema_field_to_value_layout` (type-name → `ValueLayout` bridge), `CapabilityOutputSchema::declared_value_layout` (schema → `Option<ValueLayout>`), and `CapabilityOutputSchema::validate_bytes_response` (schema-gated raw-bytes validation that produces `StructuredValue::Bytes { ptr: 0, len }` without reading byte content). **Wave 15A** wired `validate_bytes_response` into `RuntimeHost::call_capability`: when the registered output schema declares `ValueLayout::Bytes`, the host calls `validate_bytes_response` (shape-only, no content read) instead of the text `validate()` path; all other schemas continue to use `validate()`. See `docs/abi-value-contract.md §Schema-to-ValueLayout bridge`. |
...
| Secret vault | Provider abstraction + audit observability implemented. `SecretProvider` (`crates/ail-runtime/src/secret.rs`) is a `Send + Sync` trait with `resolve(vault_path) -> Result<Vec<u8>, SecretProviderError>`. `SecretProviderError` has two variants: `NotFound` (path absent) and `Unavailable` (transient failure). `SecretVault` (in-memory `HashMap`) implements it; future adapters (HashiCorp Vault, AWS Secrets Manager, etc.) implement the same trait. `SecretReadHandler::new` is generic over `P: SecretProvider + 'static`. The handler maps provider errors to generic audit categories (`"secret.not_found"` for both mapping-miss and `NotFound`; `"secret.provider_unavailable"` for `Unavailable`) recorded in `AuditEvent::CapabilityCallExecuted::denial_category`. All callers receive only opaque `HostError::CapabilityDenied("secret access denied")` — the category is audit-only, never returned to callers. Audit events record only BLAKE3 hashes of input/output. Grant checks apply on both dispatch paths. End-to-end coverage: `crates/ail-runtime/tests/secret_vault_tests.rs`, `secret_provider_audit_tests.rs`, `secret_wasm_e2e_tests.rs`. External vault adapter implementations remain future work. |
...
Code references: `crates/ail-runtime/src/host.rs`, `crates/ail-runtime/src/handler.rs`, `crates/ail-runtime/src/codec.rs`, `crates/ail-runtime/src/schema.rs`, `crates/ail-runtime/tests/effect_runtime_tests.rs`, `crates/ail-runtime/tests/typed_abi_tests.rs`, `crates/ail-runtime/tests/schema_typed_output_tests.rs`.
